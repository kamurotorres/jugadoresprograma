...
jugadorrutas.get('/listarjugadores', function (req, res) {
    //Buscar los jugadores
    try{
        jugadormodelos.find({}, function (error, jug) {
            if (error) {
                return res.status(401).send({
                    estado: "error",
                    msg: "jugadores no encontrados"
                });
            }
            else {
                if (jug != null) {
                    ...
                    res.status(401).send({
                        estado: "error",
                        msg: "jugadores no encontrados por"
                    });
                }
            }
